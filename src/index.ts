...
import { reduceDeterministic, type RtkResult } from "./rtk.js";
import { commandFromInput, contentFromText, extractText, replaceTextPreservingNonText, type ToolMetadata } from "./text.js";
...
const stats: GovernorStats = { seen: 0, changed: 0, archived: 0, policyCalls: 0, rawChars: 0, finalChars: 0 };

export default function toolResultGovernor(pi: ExtensionAPI) {
	...
	pi.on("tool_result", async (event: any, ctx) => {
		...
		if (!toolEnabled(toolName, config)) return;
		const extracted = extractText(event.content);
		if (!extracted.hadText || extracted.text.length === 0) return;
		...
		if (!result) return;
		return {
			content: replaceTextPreservingNonText(event.content, result.finalText) as any,
			details: {
...
export async function processToolResult(
	...
	let archiveHandle: ArchiveHandle | undefined;
	try {
		...
		if (archiveHandle) stats.archived++;
	} catch {
	...
	if (ctx && shouldAskPolicy(config, rawSource.text, rtk.text)) {
		...
		try {
			...
			const policy = await decideWithSmallModel(
				config,
				ctx,
				{ toolName: metadata.toolName, command, rawText: rawSource.text, rtkText: rtk.text, rtkStrategy: rtk.strategy },
				policySignal,
			);
			if (policy) {
				decision = policy;
	...
	const finalText =
		decision.action === "keep" && rawSource.text !== rawText
	...
	if (config.dryRun || finalText === rawText) return undefined;
	stats.changed++;
...
function resolveCompleteRawText(displayText: string, details: unknown, toolName: string): { text: string; complete: boolean } {
	const fullOutputPath = details && typeof details === "object" ? (details as { fullOutputPath?: unknown }).fullOutputPath : undefined;
	...
	if (candidate) {
		try {
			if (toolName !== "bash" || !isTrustedBashFullOutputPath(candidate.path)) return { text: displayText, complete: false };
			if (!existsSync(candidate.path)) return { text: displayText, complete: false };
	...
	const truncation = details && typeof details === "object" ? (details as { truncation?: { truncated?: boolean } }).truncation : undefined;
	if (truncation?.truncated) return { text: displayText, complete: false };
	return { text: displayText, complete: true };
}

function extractBashFullOutputPath(displayText: string): { path: string; visibleText: string } | undefined {
...
function visibleTextMatchesFullOutput(visibleText: string, fullText: string): boolean {
	const normalizedVisible = visibleText.trim();
	if (normalizedVisible.length < 16) return false;
	const probe = normalizedVisible.slice(Math.max(0, normalizedVisible.length - 512));
	return fullText.includes(probe);
}

function isTrustedBashFullOutputPath(path: string): boolean {
...
function makeTimeoutSignal(parent: AbortSignal | undefined, timeoutMs: number): AbortSignal | undefined {
	if (parent?.aborted) return parent;
	if (typeof AbortSignal === "undefined" || typeof AbortSignal.timeout !== "function") return parent;
