...
export async function decideWithSmallModel(
	config: GovernorConfig,
	ctx: ExtensionContext,
	input: PolicyInput,
	signal?: AbortSignal,
): Promise<GovernorDecision | undefined> {
	...
	if (!decision || decision.confidence < 0.45) return undefined;
	return decision;
}

export function shouldAskPolicy(config: GovernorConfig, rawText: string, rtkText: string): boolean {
	if (!config.model) return false;
	if (rawText.length < config.minCharsForModel) return false;
	if (rtkText.length <= config.maxSummaryChars && rtkText.length < rawText.length * 0.4) return false;
	return true;
}

export function fallbackDecision(rawText: string, rtkText: string, fallback: GovernorConfig["fallback"]): GovernorDecision {
