...
type SecretPattern = { pattern: RegExp; replacement: string | ReplacementFn };
...
export function redactLikelySecrets(text: string): RedactionResult {
	let redacted = text;
	let count = 0;
...
export function redactLikelySecretsInValue(value: unknown, seen = new WeakSet<object>(), key?: string): ValueRedactionResult {
	if (key && isSensitiveKey(key) && value !== undefined && value !== null) return { value: "[REDACTED_SECRET]", count: 1 };
	if (typeof value === "string") {
		const redacted = redactLikelySecrets(value);
		return { value: redacted.text, count: redacted.count };
	}
	if (Array.isArray(value)) {
		if (seen.has(value)) return { value: "[REDACTED_CIRCULAR]", count: 1 };
		seen.add(value);
		let count = 0;
...
function redactMultilineSecret(label: string): (match: string) => string {
	return (match) => match.split(/\r?\n/).map(() => `[${label}]`).join("\n");
}

function preserveLineCount(match: string, replacement: string): string {
	const lineCount = match.split(/\r?\n/).length;
	if (lineCount <= 1) return replacement;
