...
export function reduceDeterministic(rawText, context) {
    ...
    if (isGitStatus(command))
        semanticCandidates.push({ name: "git-status", text: compactGitStatus(text), confidence: 0.94 });
    if (isGitLog(command))
        semanticCandidates.push({ name: "git-log", text: compactGitLog(text), confidence: 0.9 });
    if (isGitDiff(command))
        semanticCandidates.push({ name: "git-diff", text: compactGitDiff(text), confidence: 0.78 });
    if (isTestCommand(command))
        semanticCandidates.push({ name: "test-output", text: compactTestOutput(text), confidence: 0.82 });
    if (isBuildOrLintCommand(command) || diagnosticOutput)
        semanticCandidates.push({ name: "build-lint", text: compactBuildAndLint(text), confidence: 0.8 });
    ...
    if (context.toolName === "ls" || context.toolName === "find" || startsCommand(command, "(?:ls|find|fd|tree|eza)"))
        semanticCandidates.push({ name: "listing", text: compactListing(text), confidence: 0.72 });
...
function isGitStatus(command) {
    return startsCommand(command, "git\\s+status");
}
function isGitLog(command) {
    return startsCommand(command, "git\\s+log");
}
function isGitDiff(command) {
    return startsCommand(command, "git\\s+(?:diff|show)");
}
function isTestCommand(command) {
    return startsCommand(command, "(?:pytest|python\\s+-m\\s+pytest|vitest|jest|mocha|cargo\\s+test|go\\s+test|deno\\s+test|npm\\s+(?:run\\s+)?test|npm\\s+t|pnpm\\s+(?:run\\s+)?test|yarn\\s+(?:run\\s+)?test|bun\\s+(?:run\\s+)?test)");
}
function isBuildOrLintCommand(command) {
    return startsCommand(command, "(?:tsc|eslint|biome|ruff|cargo\\s+clippy|cargo\\s+build|npm\\s+run\\s+build|pnpm\\s+(?:run\\s+)?build|bun\\s+(?:run\\s+)?build|yarn\\s+(?:run\\s+)?build|npx\\s+(?:tsc|eslint|biome)|bunx\\s+(?:tsc|eslint|biome)|pnpm\\s+exec\\s+(?:tsc|eslint|biome))");
}
function startsCommand(command, body) {
    return new RegExp(`(?:^|[;&|]{1,2}\\s*)(?:env\\s+)?(?:[A-Za-z_][A-Za-z0-9_]*=[^\\s]+\\s+)*(?:${body})(?:\\s|$)`).test(command);
}
function isReadLikeCommand(command) {
    return startsCommand(command, "(?:cat|sed|nl|head|tail|awk|jq|less|more)");
}
function pickSemanticCandidate(candidates, rawLength) {
    return candidates
...
function pickShortestCandidate(candidates, rawLength) {
    return candidates
        .filter((candidate) => candidate.text.trim().length > 0 && candidate.text.length < rawLength * 0.95)
...
function looksLikeDiagnosticOutput(text) {
    const lines = text.split(/\r?\n/).filter((line) => line.trim().length > 0).slice(0, 240);
    if (lines.length < 3)
        return false;
    let matches = 0;
    for (const line of lines) {
        if (/^[^:\n]*(?:\/|\b\w+\.(?:[cm]?[jt]sx?|json|md|py|rs|go|java|css|scss|html|sh|yml|yaml|toml))[^:\n]*:\d+(?::\d+)?:.*\b(error|warning|TS\d{4}|ERR|fatal|undefined|cannot find|failed)\b/i.test(line))
            matches++;
    }
    return matches >= 3;
}
...
function compactGitPorcelainStatus(text) {
    const lines = text.split(/\r?\n/).filter(Boolean);
    if (lines.length === 0)
        return undefined;
    if (!lines.every((line) => /^(##|[ MTADRCU?!]{1,2}\s+)/.test(line)))
        return undefined;
    ...
    if (omitted > 0)
        out.push(`[shrinkage: ${omitted} status entries omitted; raw archive has full list]`);
    return appendOmission(out.join("\n"), text, "porcelain status capped");
}
function compactGitLog(text) {
    return text
        .split(/\r?\n/)
        .filter((line) => /^(commit\s+[a-f0-9]+|[a-f0-9]{7,}\s+)/i.test(line.trim()) ||
...
function compactGenericText(text) {
    const out = ["Generic compacted output:"];
...
function extractPathLikeFacts(text) {
    const facts = new Set();
    ...
    for (const match of text.matchAll(regex)) {
        ...
        if (fact.length >= 4 && fact.length <= 180)
            facts.add(fact);
        if (facts.size >= 240)
            break;
    }
...
function extractInterestingLines(text) {
    ...
    for (const line of text.split(/\r?\n/)) {
        ...
        if (trimmed.length < 12)
            continue;
        if (!/(fail|failed|error|expected|actual|assert|panic|traceback|exception|warning|TS\d{4}|cannot find|undefined|not ok|fatal|denied|missing)/i.test(trimmed))
            continue;
        if (seen.has(trimmed))
            continue;
        seen.add(trimmed);
...
function compactTestOutput(text) {
    const lines = text.split(/\r?\n/);
    const interesting = lines.filter((line) => /(fail|failed|error|expected|actual|assert|panic|traceback|\bnot ok\b|\bFAIL\b)/i.test(line));
...
function compactBuildAndLint(text) {
    const lines = text.split(/\r?\n/);
    const interesting = lines.filter((line) => /(error|warning|TS\d{4}|eslint|ruff|clippy|failed|cannot find|undefined|traceback)/i.test(line));
...
function compactSearch(text) {
    ...
    for (const line of text.split(/\r?\n/)) {
        ...
        if (group.samples.length < 3)
            group.samples.push(line);
        groups.set(file, group);
    }
    if (groups.size === 0)
        return truncateChars(text, COMPACT_BUDGET);
    const out = [`Search results: ${groups.size} files, ${totalMatches} matches`, "", "Files with matches:"];
    let omittedFiles = 0;
    ...
    if (omittedFiles > 0)
        out.push(`  [shrinkage: ${omittedFiles} matching files omitted from active context; raw archive has full list]`);
    out.push("", "Sample matches:");
    let omittedSamples = 0;
    for (const [, group] of groups) {
        for (const sample of group.samples) {
            if (!pushWithinBudget(out, `  ${sample}`, COMPACT_BUDGET))
                omittedSamples++;
        }
    }
    if (omittedSamples > 0)
        out.push(`[shrinkage: ${omittedSamples} sample matches omitted from active context]`);
    return appendOmission(out.join("\n"), text, "search matches grouped; per-file samples capped");
}
function compactListing(text) {
    const lines = text.split(/\r?\n/).filter(Boolean);
    ...
    const out = [`Listing: ${lines.length} entries`];
...
function pushWithinBudget(out, line, budget) {
    const current = out.join("\n").length;
    if (current + line.length + 1 > budget)
        return false;
    out.push(line);
    return true;
}
function pushSection(out, title, lines, budget) {
    ...
    if (omitted > 0)
        out.push(`[shrinkage: ${omitted} entries omitted from ${title.toLowerCase()}]`);
}
function isImportantListingEntry(line) {
...
function collectIndentedAfter(text, marker) {
    const lines = text.split(/\r?\n/);
...
function formatFileGroup(label, files) {
    return files.length ? `${label}: ${files.length} file(s)\n  ${files.join("\n  ")}` : "";
}
function appendOmission(compacted, original, reason) {
    if (!compacted.trim())
        return truncateChars(original, 12_000);
